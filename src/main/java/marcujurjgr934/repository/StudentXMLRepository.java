...
public class StudentXMLRepository extends AbstractXMLRepository<String, Student> {
    ...
    protected Element getElementFromEntity(Student student, Document XMLdocument) {
        ...
        element.appendChild(createElement(XMLdocument, "ProfessorName", student.getProfessorName()));
    ...
    protected Student getEntityFromNode(Element node) {
        ...
        String professorName = node.getElementsByTagName("ProfessorName").item(0).getTextContent();

        return new Student(ID, name, group, email, professorName);
    }
}
